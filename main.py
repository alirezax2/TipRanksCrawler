...
def get_tiprank_value(ticker):
    ...
    headers = {"User-Agent": selected_browser_ua}
...
def get_tiprank_values(ticker):
    ...
    if response.status_code == 200:
        ...
        elements["AveragePriceTarget"] = float(
            soup.select_one(".colorblack.fonth10_semibold").text[1:]
        )
        elements["SmartScore"] = int(
            soup.select_one(".w_pxsmall60.mxauto.fontWeightbold.fontSizelarge").text
        )
...
DFUSA = load_hf_dataset(
    "america.csv", HF_TOKEN_TIPRANKS, dataset_name_TradingView_input
)
